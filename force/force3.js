...
myJson={"nodes":[{"name":"Myriel","group":1},
...
myJson.nodes.forEach(function(n) {
	labelAnchors.push({node: n});
	labelAnchors.push({node: n});
})

myJson.links.forEach(function(l) {
	labelAnchorLinks.push({source:l.source*2, target:l.source*2+1, weight:1});
})
...
var force = d3.layout.force().size([w, h]).nodes(myJson.nodes).links(myJson.links).gravity(1).linkDistance(50).charge(-3000).linkStrength(function(x) {
	return x.value * 10
});
