...
var accounts=[{name:"evajoly", img:"Joly_small.jpg", cat:"Joly"},
	      ...
	      {name:"SauvageLaurence", img:"SauvageLaurence_small.jpg",cat:"FDG"}
	      ];
      

categories.forEach(function(c,i) {accounts.filter(function(a) {return a.cat==c.short;}).forEach(function(a) {a.catId=i;})})
var hashaccount=d3.nest().key(function(d) {return d.name.toLowerCase();}).map(accounts);

accounts.forEach(function(d,i) {hashaccount[d.name.toLowerCase()]=i;});
...
function useTweet(tweet,l) {
	if(!tweet) {
		...
		d3.select(".tweetimg").attr("src","./profilepics/"+accounts[hashaccount[tweet.account.toLowerCase()]].img)
			.style("cursor","pointer")
			.style("display","block")
			.on("click", function() {cat3=accounts[hashaccount[tweet.account.toLowerCase()]].catId; initPage3();})
			;
...
d0=days[0].split("-");if(d0[2]>15&&d0[2]<27){
...
d2=days[44].split("-");if(d2[2]>4&&d2[2]<15){
...
if(d2[1]!=(d0[1]+1)){
	xAxis.append("svg:text").attr("x",function() {return (days.indexOf([(d2[1]>1?d2[0]:d0[0]),((parseFloat(d2[1])+11)%12||12),15].join("-"))*15);}).attr("y",-5).text(function() {return month[((parseFloat(d2[1])+11)%12||12)];}).classed("axismonth",1)
}
...
if(d2[1]!=(d0[1]+1)){
	tl.append("svg:text").attr("x",function() {return (days.indexOf([(d2[1]>1?d2[0]:d0[0]),((parseFloat(d2[1])+11)%12||12),15].join("-"))*15);}).attr("y",-5).text(function() {return month[((parseFloat(d2[1])+11)%12||12)];}).classed("axismonth",1)
}
...
PTA=PTA.filter(function(t) {return !t.attacks[camp[t.account.toLowerCase()]];})

var matrix=d3.range(6).map(function(c) {return d3.range(6).map(function(r) {return PTA.filter(function(t) {return (camp[t.account.toLowerCase()]==c)&&(t.attacks[r]);}).length;});})
...
PTA.forEach(function(t) {attackers[camp[t.account.toLowerCase()]].values.push(t);})
...
var rect=row.append("svg:rect").attr("x",1).attr("y",1).attr("width",(300/5)-2).attr("height",(250/5)-2).attr("stroke","none")
	.on("click", function(d) {useTweetr(PTA.filter(function(t) {return (camp[t.account.toLowerCase()]==d[0])&&(t.attacks[d[1]]);}));})
...
var gridAttack=chart2.selectAll(".gridA").data(attackers).enter().append("svg:g").classed(".gridA",1)
	...
	var rectAttack=gridAttack.append("svg:rect").attr("x",0).attr("y",0).attr("height",20)
		...
		.style("cursor","pointer")
		.on("click", function(d,i) {useTweetr(PTA.filter(function(t) {return camp[t.account.toLowerCase()]==i;}));})
		.attr("width",0)
